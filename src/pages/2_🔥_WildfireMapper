...
# Set the app title
    ...
    with st.sidebar:
        ...
        stateNames = admin1.filter(ee.Filter.eq('ADM0_NAME', "United States of America")).aggregate_array('ADM1_NAME').sort()

        selected_state = st.selectbox("Select a state", stateNames.getInfo())

        # Use the state code to filter the counties dataframe
        countyNames = admin2.filter(ee.Filter.eq('ADM1_NAME', selected_state)).aggregate_array('ADM2_NAME').sort()
    ...
    if submitted:
        ...
        with st.spinner("Processing satellite imagery for state "+ selected_state + " ..."):

            #container = st.empty()

            #st.write(selected_county,selected_year)
            fire,county_geo,fire_vector = get_wildfire(selected_state,selected_county,start_date,end_date)
            firesVis = {"min": 325.0,"max": 400.0,"palette": ['yellow', 'orange', 'red']}
            ...
            if fire:
                ...
                try:
                    ...
                    if building_list is None or building_list.loc[0][0] is None:
                        ...
                        st.write("The number of buildings affected by this wildfire:"+ str(building_list.loc[0][1]))
