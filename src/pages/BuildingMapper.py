...
def app():
    ...
    with st.sidebar:
        ...
        state_county_data = pd.read_csv(
            "https://raw.githubusercontent.com/dojowahi/ee-on-streamlit/main/data/usa_state_county.csv",
        ...
        counties = sorted(
            state_county_data.loc[state_county_data["state"] == selected_state][
                "county"
            ].unique()
        )
        ...
        geoid = state_county_data.loc[
            (state_county_data["county"] == selected_county)
            & (state_county_data["state"] == selected_state),
            "fips",
        ].iloc[0]
    ...
    if button:
        with st.spinner(
            "Processing satellite imagery for county " + selected_county + " ..."
        ):
            ...
            filtered_county = gee_counties.filter(
                ee.Filter.And(
                    ee.Filter.eq("NAMELSAD", selected_county),
